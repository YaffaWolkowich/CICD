...
@patch(
    "project.read_log_analytics.return_seconds",
    Mock(return_value=datetime.date(2023, 12, 12)),
)
...
def test_get_workspace_table_check_Logs_query_client(create_log_query_client):
    ...
    create_log_query_client().query_workspace.assert_called_once_with(
        workspace_id="workspace_id", query="Storagetype_queryLogs", timespan="30"
    )
...
@patch("project.read_log_analytics.timedelta", Mock(return_value="30"))
@patch(
    "project.read_log_analytics.create_log_query_client",
    Mock(return_value=log_query_client()),
)
...
@patch(
    "project.read_log_analytics.DefaultAzureCredential",
...
def test_create_log_query_client(LogsQueryClient):
    create_log_query_client()
    LogsQueryClient.assert_called_once_with(credential="default azure credential")
...
def test_get_times_logs():
    assert get_times_logs([mock_table(), mock_table(), mock_table()]) == [
        1,
        2,
        3,
        4,
...
def test_get_times_logs_per_table():
    assert get_times_logs_per_table(mock_table()) == [
        {"storage_name": 2, "time": 1},
        {"storage_name": 2, "time": 1},
    ]
...
def test_get_max_times_log_each_storage():
    assert get_max_times_log_each_storage(
        ...
        {"storage_name": "storage1", "time": str(mock_time(2023, 11, 12))},
