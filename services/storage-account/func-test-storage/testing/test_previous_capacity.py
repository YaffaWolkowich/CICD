...
@patch("project.previous_capacity.freq_automation_test_type", "weeks")
@patch("project.previous_capacity.freq_automation_test_number", "2")
@patch("project.previous_capacity.time_index_for_check_used_capacity", "weeks")
@patch("project.previous_capacity.time_period_for_check_used_capacity", "2")
...
@patch(
    "project.previous_capacity.get_connection_string_from_keyvalut",
    Mock(return_value="connection_string"),
)
@patch(
    "project.previous_capacity.calculate_start_and_end_dates",
    Mock(return_value=(date(2023, 7, 12), date(2023, 7, 11))),
)
@patch(
    "project.previous_capacity.query_entities_between_two_dates",
    Mock(return_value=[{"entity": 1}, {"entity": 2}, {"entity": 3}]),
)
@patch("project.previous_capacity.get_nearest_date_entity")
def test_get_storage_used_capacity_information(get_nearest_date_entity):
    get_storage_used_capacity_information("myfirsttrail")
    get_nearest_date_entity.assert_called_once_with(
...
@patch(
    "project.previous_capacity.get_nearest_date_entity",
...
@patch(
    "project.previous_capacity.datetime",
...
@patch(
    "project.previous_capacity.get_first_entity",
    Mock(return_value={"test_date": "2023-08-15 00:00:00.000+00:00"}),
)
@patch(
    "project.previous_capacity.convert_to_date_type_mil_seconds",
    Mock(return_value=date(2023, 8, 15)),
)
@patch(
    "project.previous_capacity.calculate_diff_between_two_dates",
    Mock(return_value=57),
)
@patch("project.previous_capacity.find_nearest_date_entity")
